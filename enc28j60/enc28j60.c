...
extern spi_device_t *enc28j60_spi;
...
static uint8_t command(enc_device_t *dev, uint8_t first, uint8_t second)
{
	...
	SPI_transfer_byte_cs_off(enc28j60_spi, first);
	result = SPI_transfer_byte_cs_off(enc28j60_spi, second);
...
void enc_RBM(enc_device_t *dev, uint8_t *dest, uint16_t start, uint16_t length)
{
	...
	SPI_set_cs(enc28j60_spi, SPI_COMMON_LOW);
	SPI_transfer_byte_cs_off(enc28j60_spi, 0x3a);

	while(length--)
		*(dest++) = SPI_transfer_byte_cs_off(enc28j60_spi, 0);
...
{
	SPI_set_cs(enc28j60_spi, SPI_COMMON_LOW);
	SPI_transfer_byte_cs_off(enc28j60_spi, 0x7a);

	while(length--)
		SPI_transfer_byte_cs_off(enc28j60_spi, *(src++));
...
void enc_SRC(enc_device_t *dev) {
	SPI_transfer_byte_cs_off(enc28j60_spi, 0xff);
	OMM_busy_delay(10);
}
